...
@section('content')
                        ...
                        <form method="post" action="{{ route('blog.post.update', $row->id) }}">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" name="english_title"
                                    placeholder="row english Title" value="{{ $row->english_title }}">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" name="bangla_title"
                                    placeholder="row Bangla Title" phn value="{{ $row->bangla_title }}">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" name="comment">
                                    <option value="yes" {{ $row->comment === 'yes' ? 'selected' : '' }}>Yes</option>
                                    <option value="no" {{ $row->comment === 'no' ? 'selected' : '' }}>No</option>
                                </select>
                                @if ($errors->has('visibility'))
                                    <span class="text-danger">{{ $errors->first('visibility') }}</span>
                                @endif
                            ...
                            <div class="form-group mb-3">
                                ...
                                <select class="select2 form-control" name="category[]" multiple="multiple" id="cat">
                                    @foreach ($categories as $category)
                                        <option value="{{ $category->id }}"
                                            {{ in_array($category['id'], $selectedCategory) ? 'selected' : '' }}>
                                            {{ $category->english_title }} </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-3">
                                ...
                                <textarea name="bangla_description" class="form-control">{{ $row->bangla_description }}</textarea>
                            ...
                            <div class="mb-3">
                                ...
                                <textarea name="english_description" class="form-control">{{ $row->english_description }}</textarea>
    ...
    @section('page-script')
        <script>
            ...
            CKEDITOR.replace('bangla_description', {
                height: 500
            });
